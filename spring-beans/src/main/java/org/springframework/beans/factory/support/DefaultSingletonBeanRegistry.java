 ...
 *
 * <p>共享 bean 实例的通用注册表，实现 {@link org.springframework.beans.factory.config.SingletonBeanRegistry}。
 * 允许注册应为注册表的所有调用者共享的单例实例，通过 bean 名称获取。
 * <p>还支持注册 {@link org.springframework.beans.factory.DisposableBean}
 * 实例（可能对应也可能不对应注册的单例），在注册表关闭时销毁。
 ...
 * <p>请注意，与 {@link AbstractBeanFactory} 和 {@link DefaultListableBeanFactory}（继承自它）相比，该类既不假定 bean 定义概念，也不假定 bean 实例的特定创建过程。
 * 也可以用作嵌套的助手来委托。
 *
...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	...
	/** Maximum number of suppressed exceptions to preserve.要保留的最大抑制异常数 */
